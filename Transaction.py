...
# NOTE: the structure looks RECURSIVE
# which is pretty complicated to trace


class Transaction:

    def __init__(
        self, prev_txn, scriptSign, amnt, receiverPubKeyHash, genesisBlockTxn=False
    ):
        # NOTE: hash of all the values inside the transaction
        allHash = ""
        # All hash
        for txn in prev_txn:
        ...
        if genesisBlockTxn:
            self.output.append(
                (amnt, ScriptPubKey(receiverPubKeyHash, receiverPubKeyHash.hexdigest()))
            )
        ...
        if validCoins > amnt:
            for val in self.input:
                # TODO: This is very fucking not readable
                ...
                if wallet_amt > 0:
                    senderPubKeyHash = SHA256.new(
                        hashlib.sha256(val[1].pubKey).hexdigest().encode()
                    )
                    self.output.append(
                        (
                            wallet_amt,
    ...
    # NOTE: check this function first to understand the structure of Transaction
    def isValidTxn(self, amnt):
        validBitCoins = 0
        for x in self.input:
            index = x[0][1]  # Index value ? means ?
